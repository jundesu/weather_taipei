...
function fetchCurrentObservation() {
  return fetch(
    `https://opendata.cwb.gov.tw/api/v1/rest/datastore/O-A0003-001?Authorization=${process.env.REACT_APP_CWB_API_KEY}&locationName=${locationName}&format=JSON`
  )
    .then((response) => {
...
function fetchCurrentWeatherType() {
  return fetch(
    `https://opendata.cwb.gov.tw/api/v1/rest/datastore/F-C0032-001?Authorization=${process.env.REACT_APP_CWB_API_KEY}&format=JSON&locationName=臺北市`
  )
    .then((response) => {
      return response.json();
    })
...
function fetchSunriseSunset() {
  ...
  return fetch(
    `https://opendata.cwb.gov.tw/api/v1/rest/datastore/A-B0062-001?Authorization=${process.env.REACT_APP_CWB_API_KEY}&format=JSON&locationName=%E8%87%BA%E5%8C%97%E5%B8%82&timeFrom=${nowString}&timeTo=${tomorrowString}`
  )
    .then((response) => {
      return response.json();
    })
...
function fetchWeeklyForecast() {
  return fetch(
    `https://opendata.cwb.gov.tw/api/v1/rest/datastore/F-D0047-063?Authorization=${process.env.REACT_APP_CWB_API_KEY}&format=JSON`
  )
    .then((response) => {
      return response.json();
    })
  ...
  return (
    <div className="weatherInfo">
      ...
      <div className="temperature">
        {Math.round(props.observation.temperature || 0)}
        <span>&#8451;</span>
      </div>
    ...
    <div>
      <img
        src={handleWeatherIcon(
          props.dayOrNight,
          Number(props.weatherType.type || 0)
        )}
        ...
        return (
          <div className="day" key={date}>
            ...
            <div>
              <img
                className="fcIcon"
                src={handleWeatherIcon(props.dayOrNight, Number(type || 0))}
              />
            </div>
          </div>
        );
